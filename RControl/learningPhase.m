...
for i = 1:initialRunlength + sampleRunlength + freeRunlength + plotRunlength 
    ...
    if linearNetwork
        if noiselevel == 0 || noiselevel == 0.0 || i > initialRunlength + sampleRunlength
            internal_state = ([W_input, inWM, W_feedback]*full_state);  
        else
            internal_state = ([W_input, inWM, W_feedback]*full_state + noiselevel * 2.0 * (rand(netDim,1)-0.5));
            % internal_state = ([W_input, inWM, W_feedback]*full_state + noiselevel * 2.0 * (randn(netDim,1)));
        end
    else
        if noiselevel == 0 || noiselevel == 0.0 || i > initialRunlength + sampleRunlength
...
%             collectObserved = [collectObserved; observed_state' ];
    ...
    if linearOutputUnits
        netOut = W_output *[internal_state;input_signal];
    else
...
% %     
    ...
    if i == initialRunlength + sampleRunlength
        ...
        for j = 1:outputLength
            if linearOutputUnits
                MSE_train(1,j) = sum((teachCollectMat(:,j) - ...
                    (stateCollectMat * W_output(j,:)')).^2);
            else
                MSE_train(1,j) = sum((f(teachCollectMat(:,j)) - ...
                    f(stateCollectMat * W_output(j,:)')).^2);
            end
